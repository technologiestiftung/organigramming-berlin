...
const ChartNode = ({
      ...
      if (draggedInfo) {
        setAllowedDrop(
          !document
            ...
            ? true
            : node.current.id === "n-root" ? true: false
        );
      } else {
  ...
  const toggleAncestors = (actionNode) => {
    ...
    if (isAncestorsCollapsed) {
      // Expand upward, one level only
      actionNode.classList.remove("isAncestorsCollapsed");
      node.firstChild.classList.remove("hidden");
    } else {
      // When collapsing downwards, all predecessor nodes and the siblings of the predecessor nodes are collapsed.
      const isSiblingsCollapsed = Array.from(
      ...
      // If there are still expanded ancestor nodes, then continue to collapse them
      if (
  ...
  const toggleSiblings = (actionNode) => {
    ...
    // Sibling nodes of the same level are processed first
    while (node) {
      if (isSiblingsCollapsed) {
    ...
    // Expand the parent node along with the sibling node
  ...
  return (
    <li className="oc-hierarchy">
      <div
        ref={node}
        id={datasource.id}
        className={nodeClass + " " + (datasource.style ? datasource.style : "")}
        draggable={draggable ? "true" : undefined}
        onClick={datasource.style !== "root" ? clickNodeHandler : null}
        onDragStart={datasource.style !== "root" ? dragstartHandler : null}
              ...
              <div className="oc-content">
                <ul className="departments">
                  {datasource.departments &&
                    datasource.departments.map((department) => (
                      <li key={datasource.name + "-" + department.name}>
                ...
                {datasource.address.street && (
                  <div className="address">
                    ...
                    {datasource.address.street}{" "}
                    {datasource.address.housenumber && " "}
                    {datasource.address.housenumber}
                    {datasource.address.zipCode && <br />}
                    {datasource.address.zipCode}
      ...
      {datasource.organisations && datasource.organisations.length > 0 && (
        <ul className={isChildrenCollapsed ? "hidden" : ""}>
          {datasource.organisations.map((node) => (
            <ChartNode
              datasource={node}
