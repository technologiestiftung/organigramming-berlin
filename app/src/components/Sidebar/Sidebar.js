...
const Sidebar = ({ data, sendDataUp, selected, setSelected, onExport, onSave, onUndo, onRedo, enableUndo, enableRedo}) => {
  ...
  return (
    <>
      <NewDocumentModal show={newDocumentModalShow} onHide={() => setNewDocumentModalShow(false)} sendDataUp={onChange}/>
      <Row>
        ...
        <Nav className="me-auto">
          <ButtonGroup aria-label="Toolbar">
            <Button
              variant="light"
              onClick={() => setNewDocumentModalShow(true)}>
              <svg
            ...
            <Button
              ...
              onClick={() => selected != null ? setActiveTap("organisation"): undefined}
            ...
            <Button
              variant="light"
              variant="light"
              className={activeTap === "export" ? "active" : ""}
              onClick={() => setActiveTap("export")}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
          ...
          <Button onClick={onUndo} disabled={!enableUndo}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="16"
        ...
        c0.3,0,0.5,0.1,0.6,0.4c0,0,0,0,0,0C14.3,8.1,12,11.1,8.7,11.5z"
          ...
          <Button onClick={onRedo} disabled={!enableRedo}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="16"
      ...
      >
        <div className="sidebar">
          ...
          {activeTap === "organisation" && (
            <OrganisationTab data={data} sendDataUp={onChange} selected={selected} setSelected={(e)=>setSelected(e)} />
          )}
          {activeTap === "export" && (
            <ExportTab data={data} sendDataUp={onChange} onExport={(e)=> onExport(e)} onSave={onSave} />
          )}
        </div>
        <div></div>
      </Split>
